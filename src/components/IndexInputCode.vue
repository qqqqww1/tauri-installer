<template>
  ...
  <input
    ...
    class="px-2 text-center outline-none input h-12 w-300px uppercase"
  />
  <div class="text-red-500 mt-2 text-sm flex items-center" v-if="error">
    <div class="i-mdi:close mr-1 text-xl" />
    {{ error }}
  </div>
  <q-btn
    unelevated
    class="mt-8 !rounded-xl mx-auto bg-green-custom !font-bold !px-12 !text-xl"
    text-color="white"
    label="激活"
    @click="toActive"
    :loading="loading"
    :disable="!currentCode || loading"
  />
</template>
