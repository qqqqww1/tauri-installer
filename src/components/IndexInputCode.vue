<template>
  <div class="mb-1">许可证序列号:</div>
  <!-- 强制大写 -->
  <input v-model="currentCode" border="3 solid focus:blue-5"
    class="px-2 text-center outline-none input h-10 w-250px uppercase" />
  <div class="text-red-500 mt-1 text-sm flex items-center h-0" v-if="error">
    <div class="i-mdi:close mr-1 text-xl" />
    {{ error }}
  </div>
  <q-btn unelevated class="mt-7 !min-h-8 !rounded-xl mx-auto !leading-8 bg-green-custom !font-bold !px-12 !text-lg"
